...
//
// dec 23 2017 by jhr
// jun 3 2021 by jhr, extracted from Kudocracy
//
...
TrustActor.get_ranked = function( max, depth ){
  ...
  ||  depth === "locality"
    ...
    // max times 2 to compensate for some filtering
    other_list = TrustActor.get_ranked( max * 2, 1 ).actors;
  }
  ...
  for( var ii = 0 ; ii < list.length ; ii++ ){
    ...
    // Criteria, number of followers from within the community
    }else if( depth === "local_followers" ){
      if( reverse ){
        actor.score = community_size - user.followers_count;
      }else{
        actor.score = user.followers_count;
      }
    
    // Criteria, ratio of pagerank over followers
    }else if( depth === "efficiency" ){
      if( actor.twitter_followers_rank > actor.pagerank_rank && !reverse ){
        // actor.score
        // = actor.pagerank_score
        // / ( 10 * ( user.followers_count / community_size ) );
        actor.score = actor.twitter_followers_rank / actor.pagerank_rank;
        // actor.score = actor.twitter_followers_rank - actor.pagerank_rank;
      }else if( actor.twitter_followers_rank < actor.pagerank_rank && reverse ){
        actor.score = actor.pagerank_rank / actor.twitter_followers_rank;
      }else{
        actor.score = 0;
      }
      
    // Criteria, adjusted number of twitter followers
    }else if( depth === "locality" ){
      if( reverse ){
        actor.score = 1 - user.locality_factor;
      }else{
        actor.score = user.locality_factor;
      }
  ...
  trace(
    "Top user is " + top_user, "with", top, "score.",
    "Mean:",     round( total / list.length ),
  ...
  for( ii = 0 ; ii < list.length ; ii++ ){
    list[ ii ].rank = ii + 1;
    // Keep those with enought influence to improve signal/noise ratio
    if( other_list && other_list.indexOf( list[ ii ] ) === -1 )continue;
